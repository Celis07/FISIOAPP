...
function exportPDF(patient,pres){
  if(!pres)return;
  const planDays=pres.days_structure&&pres.days_structure.length>0
    ? pres.days_structure
    : [{day:1,label:"Día 1",exercises:pres.exercises||[]}];
  ...
  const totalEx=planDays.reduce((s,d)=>s+d.exercises.length,0);
  let html=`<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"/><title>Plan – ${patient.name}</title>
  <style>*{font-family:'Inter',-apple-system,sans-serif;box-sizing:border-box;margin:0;padding:0}body{background:#fff;color:#111;padding:32px;max-width:760px;margin:0 auto}.header{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:18px;border-bottom:2px solid #0891b2;margin-bottom:22px}.logo{font-size:20px;font-weight:800;color:#0891b2}.pt-name{font-size:22px;font-weight:700}.meta{text-align:right;font-size:12px;color:#666}.day-title{background:#f0f7ff;border-left:3px solid #0891b2;padding:8px 14px;margin:18px 0 10px;font-weight:700;font-size:14px;color:#0891b2;border-radius:0 6px 6px 0}.block-label{display:flex;align-items:center;gap:6px;margin:12px 0 6px;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.8px}.dot{width:8px;height:8px;border-radius:50%;flex-shrink:0}.row{display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#f8f9fa;border-radius:7px;margin-bottom:4px}.rname{font-weight:600;font-size:13px}.rdose{font-weight:700;color:#0891b2;font-size:13px}.note{background:#e0f5fb;border-left:3px solid #0891b2;padding:10px 14px;border-radius:6px;margin-bottom:16px;font-size:13px;color:#0c4a6e}.footer{margin-top:28px;padding-top:14px;border-top:1px solid #eee;display:flex;justify-content:space-between;font-size:11px;color:#aaa}@media print{body{padding:20px}}</style></head><body>
  <div class="header"><div><div class="logo">FisioApp</div><div class="pt-name" style="margin-top:8px">${patient.name}</div><div style="font-size:13px;color:#666;margin-top:2px">${patient.condition||""}</div></div><div class="meta"><b>Plan de ejercicios</b><br/>${date}<br/>${totalEx} ejercicios · ${planDays.length} días${pres.duration_days?`<br/>${pres.duration_days} días de duración`:""}</div></div>
  ${pres.note?`<div class="note">${pres.note}</div>`:""}`;
  planDays.forEach(day=>{
    html+=`<div class="day-title">${day.label} · ${day.exercises.length} ejercicios</div>`;
    const blocks={};
    day.exercises.forEach(ex=>{const b=ex.block||"Sin bloque";if(!blocks[b])blocks[b]=[];blocks[b].push(ex);});
        ...
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 18px",overflow:"hidden"}}>
          ...
          {d.appts.length===0?<p style={{color:C.dim,fontSize:13,textAlign:"center",padding:"8px 0"}}>Sin citas</p>:d.appts.map(a=>(
            <div key={a.id} style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
              <div style={{background:"rgba(8,145,178,.1)",border:"1px solid rgba(8,145,178,.15)",borderRadius:8,padding:"3px 8px",minWidth:50,textAlign:"center",flexShrink:0}}>
        ...
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:16,marginBottom:14}}>
          ...
          <div style={{position:"relative",height:chartH+20}}>
            ...
            <svg style={{position:"absolute",left:16,right:0,top:0,bottom:20,width:"calc(100% - 16px)",height:chartH+20}} preserveAspectRatio="none">
              {chartData.length>1&&<polyline points={chartData.map((r,i)=>`${(i/(chartData.length-1))*100}%,${chartH-(r.pain_level/maxVal)*chartH}`).join(" ")} fill="none" stroke={C.danger} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>}
              {chartData.length>1&&<polyline points={chartData.map((r,i)=>`${(i/(chartData.length-1))*100}%,${chartH-(r.fatigue_level/maxVal)*chartH}`).join(" ")} fill="none" stroke={C.warn} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="4 2"/>}
          ...
          {timeline.map((entry,i)=>(
            <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:entry.note?8:0}}>
                <span style={{color:C.accentL,fontSize:12,fontWeight:600}}>{new Date(entry.date+"T12:00").toLocaleDateString("es-CO",{weekday:"short",day:"numeric",month:"short",year:"numeric"})}</span>
...
// ─── PLAN CALENDAR VIEW ───────────────────────────────────────────────────────
function PlanCalendarView({prescriptions,patient,user,loading,onPrescribe,onEdit,onDelete,fetchPrescriptions}){
  const [activePres,setActivePres]=useState(null);
  const [selectedDay,setSelectedDay]=useState(null);

  useEffect(()=>{
    if(prescriptions.length>0&&!activePres){
      setActivePres(prescriptions[0].id);
      setSelectedDay(0);
    }
  },[prescriptions]);
  ...
  const pres=prescriptions.find(p=>p.id===activePres)||prescriptions[0];
  const dl=pres.end_date?daysLeft(pres.end_date):null;
  const dCol=dl===null?null:dl<0?C.danger:dl<=5?C.warn:C.success;
  const planDays=pres.days_structure&&pres.days_structure.length>0
    ? pres.days_structure
    : [{day:1,label:"Día 1",exercises:pres.exercises||[]}];
  const dayData=selectedDay!==null&&planDays[selectedDay]?planDays[selectedDay]:null;

  const duplicatePres=async()=>{
    const dur=pres.duration_days||30;
    const sd=localDateStr(new Date());
    const ed=localDateStr(new Date(Date.now()+dur*864e5));
    await supabase.from("prescriptions").insert({patient_id:patient.id,therapist_id:user.id,exercises:pres.exercises,days_structure:pres.days_structure,note:pres.note||"",duration_days:dur,start_date:sd,end_date:ed});
    fetchPrescriptions();
  };

  const renewPres=async()=>{
    if(!window.confirm("¿Renovar por el mismo período?"))return;
    const dur=pres.duration_days||30;
    const sd=localDateStr(new Date());
    const ed=localDateStr(new Date(Date.now()+dur*864e5));
    ...
    <div>
      ...
      {prescriptions.length>1&&(
        <div style={{display:"flex",gap:6,marginBottom:12,overflowX:"auto",paddingBottom:4}}>
          {prescriptions.map((p,i)=>(
            <button key={p.id} onClick={()=>{setActivePres(p.id);setSelectedDay(0);}}
              style={{padding:"5px 12px",borderRadius:8,border:`1.5px solid ${activePres===p.id?C.accent:C.border}`,background:activePres===p.id?"rgba(8,145,178,.1)":"transparent",color:activePres===p.id?C.accent:C.muted,fontSize:12,fontWeight:activePres===p.id?700:400,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,transition:"all .15s"}}>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"12px 14px",marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            {prescriptions[0]?.id===pres.id&&<span style={{color:C.text,fontWeight:600,fontSize:13}}>Plan actual</span>}
            {dCol&&<Tag label={dl<0?"Vencido":dl===0?"Vence hoy":`${dl}d`} color={dCol}/>}
          </div>
          <Menu items={[
            {label:"Editar",       icon:I.edit,  action:()=>onEdit(pres)},
            {label:"Duplicar",     action:duplicatePres},
            {label:"Renovar",      action:renewPres},
            {label:"Exportar PDF", icon:I.pdf,   action:()=>exportPDF(patient,pres)},
            "---",
            {label:"Eliminar",     icon:I.trash, danger:true, action:()=>onDelete(pres.id)},
          ]}/>
        </div>
        <p style={{color:C.muted,fontSize:12,margin:0}}>
          {new Date(pres.created_at).toLocaleDateString("es-CO",{day:"numeric",month:"short",year:"numeric"})}
          {" · "}{planDays.length} {planDays.length===1?"día":"días"}
          {" · "}{planDays.reduce((s,d)=>s+d.exercises.length,0)} ejercicios
        </p>
        {pres.note&&<p style={{color:C.accentL,fontSize:12,background:"rgba(8,145,178,.07)",border:"1px solid rgba(8,145,178,.15)",borderRadius:8,padding:"7px 11px",marginTop:8,lineHeight:1.6,margin:"8px 0 0"}}>{pres.note}</p>}
      </div>

      {/* Calendario de días */}
      <p style={{color:C.muted,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:.8,margin:"0 0 8px"}}>Días del plan</p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(72px,1fr))",gap:8,marginBottom:14}}>
        {planDays.map((d,i)=>{
          ...
          return(
            <button key={i} onClick={()=>setSelectedDay(isSelected?null:i)}
              style={{padding:"10px 6px",borderRadius:12,border:`2px solid ${isSelected?C.accent:hasEx?"rgba(8,145,178,.2)":C.border}`,background:isSelected?"rgba(8,145,178,.12)":hasEx?"rgba(8,145,178,.04)":C.surface,cursor:"pointer",textAlign:"center",transition:"all .2s",transform:isSelected?"scale(1.04)":"scale(1)"}}>
              <div style={{fontSize:10,color:isSelected?C.accent:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:3}}>{d.label}</div>
              <div style={{fontSize:22,fontWeight:700,color:isSelected?C.accent:hasEx?C.text:C.dim,fontFamily:"'Fraunces',serif",lineHeight:1}}>{d.exercises.length}</div>
              <div style={{fontSize:9,color:isSelected?C.accent:C.dim,marginTop:2}}>ejerc.</div>
            </button>
          );
        })}
      </div>
      ...
      {/* Detalle del día */}
      {dayData&&(
        <div style={{background:C.surface,border:`1.5px solid rgba(8,145,178,.3)`,borderRadius:14,padding:14}}>
          <style>{"@keyframes fadeIn{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:none}}"}</style>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,animation:"fadeIn .2s ease"}}>
            <p style={{color:C.text,fontWeight:700,fontSize:14,margin:0}}>{dayData.label}</p>
            <span style={{color:C.muted,fontSize:12}}>{dayData.exercises.length} ejercicios</span>
          </div>
          {BLOCKS.concat(["Sin bloque"]).map(b=>{
            const exList=dayData.exercises.filter(e=>(e.block||"Sin bloque")===b);
                ...
                {exList.map((ex,j)=>(
                  <div key={j} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                    <div style={{minWidth:0,flex:1}}>
                      ...
                      {ex.description&&<p style={{color:C.muted,fontSize:11,margin:"2px 0 0",lineHeight:1.4}}>{ex.description}</p>}
                      {ex.video_url&&<a href={ex.video_url} target="_blank" rel="noreferrer" style={{display:"inline-block",color:C.accent,fontSize:11,marginTop:2,textDecoration:"none"}}>▶ ver video</a>}
                    </div>
                    <span style={{color:C.accent,fontWeight:700,fontSize:13,flexShrink:0,marginLeft:12}}>{ex.sets}×{ex.reps}</span>
                  </div>
                ))}
              </div>
            );
      ...
      {selectedDay===null&&<p style={{color:C.dim,fontSize:12,textAlign:"center",padding:"8px 0"}}>Toca un día para ver los ejercicios</p>}
    </div>
  );
}
...
function PatientProfile({patient,user,onBack,onPrescribe,onApprove}){
  ...
  const deletePrescription=async(id)=>{
    ...
    setPres(prev=>prev.filter(p=>p.id!==id));
  };
  ...
  const deletePatient=async()=>{
    if(!window.confirm(`¿Eliminar a ${patient.name}? Se borrarán todos sus datos.`))return;
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12}}>
                ...
                <div style={{display:"flex",alignItems:"flex-end",gap:6,height:60}}>
                  {lpd.map(({d,n},i)=>{const isT=d.toDateString()===new Date().toDateString();return(
                    <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                      ...
                      <div style={{width:"100%",borderRadius:6,minHeight:3,height:`${n>0?Math.max((n/maxN)*46,6):3}px`,background:isT?C.accent:n>0?"rgba(8,145,178,.3)":C.border,transition:"height .4s"}}/>
          ...
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:16}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {Array.from({length:30},(_,i)=>{const d=new Date();d.setDate(d.getDate()-(29-i));const n=logs.filter(l=>new Date(l.completed_at).toDateString()===d.toDateString()).length;const isT=d.toDateString()===new Date().toDateString();return<div key={i} style={{width:22,height:22,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,outline:isT?`2px solid ${C.accent}`:"none",outlineOffset:1,background:n===0?C.border:n<=2?"rgba(8,145,178,.22)":n<=5?"rgba(8,145,178,.5)":"rgba(8,145,178,.88)",color:n===0?C.dim:C.text}}>{d.getDate()}</div>;})}
            </div>
          </div>
        </div>
      )}
...
function PatientsView({user,onPrescribe,onViewProfile,initialFilter,onClearFilter}){
  ...
  const addPatient=async()=>{
    ...
    const{data,error}=await supabase.from("patients").insert({name:form.name,age:parseInt(form.age)||null,condition:form.condition,email:form.email,is_athlete:form.is_athlete,sport:form.sport||null,therapist_id:user.id,invite_token:token}).select().single();
...
function PrescribeView({user,patient,onBack,existingPrescription}){
  ...
  const initDays=()=>{
    if(isEdit){
      if(existingPrescription.days_structure&&existingPrescription.days_structure.length>0){
        return existingPrescription.days_structure.map(d=>({...d,exercises:[...d.exercises]}));
      }
      if(existingPrescription.exercises?.length>0){
        return [{day:1,label:"Día 1",exercises:[...existingPrescription.exercises]}];
      }
    }
    return [{day:1,label:"Día 1",exercises:[]}];
  };
  ...
  const addDay=()=>{const n=days.length+1;setDays(prev=>[...prev,{day:n,label:`Día ${n}`,exercises:[]}]);setActiveDay(days.length);};
  const duplicateDay=(idx)=>{const src=days[idx];const n=days.length+1;const copy={day:n,label:`Día ${n}`,exercises:src.exercises.map(e=>({...e}))};setDays(prev=>[...prev,copy]);setActiveDay(days.length);};
  const removeDay=(idx)=>{if(days.length===1)return;if(!window.confirm(`¿Eliminar ${days[idx].label}?`))return;const nd=days.filter((_,i)=>i!==idx).map((d,i)=>({...d,day:i+1,label:`Día ${i+1}`}));setDays(nd);setActiveDay(Math.min(activeDay,nd.length-1));};

  const isInDay=ex=>dayExercises.find(e=>e.id===ex.id);
  const blockOfInDay=ex=>{const found=dayExercises.find(e=>e.id===ex.id);return found?.block||null;};
  const addEx=ex=>{if(isInDay(ex))return;setDays(prev=>prev.map((d,i)=>i===activeDay?{...d,exercises:[...d.exercises,{...ex,sets:ex.defaultSets,reps:ex.defaultReps,block:activeBlock}]}:d));};
  const removeEx=(exId)=>{setDays(prev=>prev.map((d,i)=>i===activeDay?{...d,exercises:d.exercises.filter(e=>e.id!==exId)}:d));};
  const updateEx=(exId,field,val)=>{setDays(prev=>prev.map((d,i)=>i===activeDay?{...d,exercises:d.exercises.map(e=>e.id===exId?{...e,[field]:val}:e)}:d));};

  const allExercises=[...customExs,...EXERCISES];
  const filtered=allExercises.filter(ex=>{
    if(cat==="Mis ejercicios")return ex.isCustom;
    const mc=cat==="Todos"||ex.category===cat;
  ...
  const WARMUP_PROTOCOL={
    ...
    exercises:[
      ...
      {id:90005,name:"Rotación externa de hombro con banda",category:"Calentamiento",defaultSets:3,defaultReps:"15",sets:3,reps:"15",block:"Calentamiento / Activación",description:"Codo a 90°, rotación externa con banda"},
    ...
    note:"Protocolo de calentamiento general.",
    duration_days:30,
  };

  const applyTemplate=(tpl)=>{
    const newExs=(tpl.exercises||[]).map(e=>({...e,block:e.block||"Trabajo central"}));
    setDays(prev=>prev.map((d,i)=>i===activeDay?{...d,exercises:newExs}:d));
    if(tpl.note)setNote(tpl.note);
    if(tpl.duration_days)setDur(tpl.duration_days);
    setShowTemplates(false);
  };

  const deleteTemplate=async(id)=>{if(!window.confirm("¿Eliminar esta plantilla?"))return;await supabase.from("prescription_templates").delete().eq("id",id);setTemplates(prev=>prev.filter(t=>t.id!==id));};

  const saveTemplate=async()=>{
    if(!templateName.trim())return;setSavingTemplate(true);
    await supabase.from("prescription_templates").insert({therapist_id:user.id,name:templateName.trim(),exercises:currentDay.exercises,note,duration_days:duration});
    const{data}=await supabase.from("prescription_templates").select("*").eq("therapist_id",user.id).order("created_at",{ascending:false});
    setTemplates(data||[]);setTemplateName("");setShowSaveTemplate(false);setSavingTemplate(false);
  };

  const saveNewEx=async()=>{
    if(!newEx.name.trim())return;setSavingEx(true);
    ...
    if(data){
      const ex={id:"custom_"+data.id,dbId:data.id,name:data.name,description:data.description||"",category:data.category,defaultSets:data.default_sets,defaultReps:data.default_reps,video_url:data.video_url||null,isCustom:true};
      setDays(prev=>prev.map((d,i)=>i===activeDay?{...d,exercises:[...d.exercises,{...ex,sets:ex.defaultSets,reps:ex.defaultReps,block:activeBlock}]}:d));
      refreshCustom();
    }
  ...
  const send=async()=>{
    ...
    const legacyExs=days[0]?.exercises||[];
    const payload={exercises:legacyExs,days_structure:days,note,duration_days:duration};
    let error;
    if(isEdit){({error}=await supabase.from("prescriptions").update({...payload,end_date:ed}).eq("id",existingPrescription.id));}
    else{({error}=await supabase.from("prescriptions").insert({patient_id:patient.id,therapist_id:user.id,...payload,start_date:sd,end_date:ed}));}
  ...
  if(submitted)return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 16px",textAlign:"center"}}>
      ...
      <p style={{color:C.muted,marginBottom:6}}>{days.length} días · {totalExercises} ejercicios · {patient.name}</p>
      <Btn onClick={onBack} variant="primary" style={{marginTop:12}}>Volver</Btn>
    </div>
  );
    ...
    <div>
      {showNewEx&&(
        <Modal title="Nuevo ejercicio" onClose={()=>setNewExOpen(false)}>
          <div style={{display:"grid",gap:10}}>
            ...
            <div><label style={{fontSize:12,color:C.muted,display:"block",marginBottom:4}}>URL video (opcional)</label><input value={newEx.video_url||""} onChange={e=>setNewEx({...newEx,video_url:e.target.value})} placeholder="https://youtube.com/..." style={inp}/></div>
          </div>
          <p style={{color:C.muted,fontSize:12,marginTop:10}}>Se agregará a: <strong style={{color:C.accentL}}>{activeBlock}</strong> · {currentDay.label}</p>
      ...
      {showTemplates&&(
        <Modal title="Plantillas" onClose={()=>setShowTemplates(false)}>
          <p style={{color:C.muted,fontSize:12,marginBottom:12}}>Se aplican al <strong style={{color:C.text}}>{currentDay.label}</strong></p>
          <div style={{marginBottom:14}}>
            <p style={{color:C.muted,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:.8,margin:"0 0 8px"}}>Base</p>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 14px"}}>
              <div><p style={{color:C.text,fontWeight:600,fontSize:13,margin:0}}>Calentamiento general</p><p style={{color:C.muted,fontSize:11,margin:"2px 0 0"}}>8 ejercicios</p></div>
              ...
              <div style={{display:"grid",gap:6}}>
                {templates.map(t=>(
                  <div key={t.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 14px"}}>
                    <div><p style={{color:C.text,fontWeight:600,fontSize:13,margin:0}}>{t.name}</p><p style={{color:C.muted,fontSize:11,margin:"2px 0 0"}}>{(t.exercises||[]).length} ejerc. · {t.duration_days}d</p></div>
          ...
          {templates.length===0&&<p style={{color:C.muted,fontSize:13,textAlign:"center",padding:"8px 0"}}>Aún no tienes plantillas guardadas</p>}
        </Modal>
      )}

      {showSaveTemplate&&(
        <Modal title="Guardar como plantilla" onClose={()=>setShowSaveTemplate(false)} maxWidth={360}>
          <p style={{color:C.muted,fontSize:13,marginBottom:14}}>Guarda los ejercicios de <strong>{currentDay.label}</strong> ({currentDay.exercises.length} ejercicios) como plantilla.</p>
          <input value={templateName} onChange={e=>setTemplateName(e.target.value)} placeholder="Nombre de la plantilla" style={{...inp,marginBottom:14}} onKeyDown={e=>e.key==="Enter"&&saveTemplate()}/>
          <div style={{display:"flex",gap:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,gap:8,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          {currentDay.exercises.length>0&&<Btn onClick={()=>setShowSaveTemplate(true)} variant="subtle" style={{fontSize:12}}>Guardar plantilla</Btn>}
        </div>
      </div>

      <p style={{color:C.muted,fontSize:13,margin:"0 0 14px"}}>
        Plan para <strong style={{color:C.text}}>{patient.name}</strong> · {days.length} {days.length===1?"día":"días"} · {totalExercises} ejercicios
      </p>
          ...
          {days.map((d,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:0,flexShrink:0}}>
              <button onClick={()=>setActiveDay(i)}
                style={{padding:"7px 14px",borderRadius:days.length>1?"10px 0 0 10px":10,border:`1.5px solid ${activeDay===i?C.accent:C.border}`,borderRight:days.length>1?"none":"1.5px solid "+(activeDay===i?C.accent:C.border),background:activeDay===i?"rgba(8,145,178,.12)":"transparent",color:activeDay===i?C.accent:C.muted,fontSize:12,fontWeight:activeDay===i?700:500,cursor:"pointer",whiteSpace:"nowrap",transition:"all .15s"}}>
                {d.label}
                {d.exercises.length>0&&<span style={{marginLeft:5,fontSize:10,color:activeDay===i?C.accent:C.dim,fontWeight:600}}>{d.exercises.length}</span>}
              </button>
              {days.length>1&&(
                <div style={{display:"flex",flexDirection:"column",border:`1.5px solid ${activeDay===i?C.accent:C.border}`,borderLeft:"none",borderRadius:"0 10px 10px 0",overflow:"hidden"}}>
                  <button onClick={()=>duplicateDay(i)} title="Duplicar día" style={{padding:"3px 7px",background:"transparent",border:"none",color:C.dim,cursor:"pointer",fontSize:11,borderBottom:`1px solid ${C.border}`,lineHeight:1.2}}>⧉</button>
                  <button onClick={()=>removeDay(i)} title="Eliminar día" style={{padding:"3px 7px",background:"transparent",border:"none",color:C.dim,cursor:"pointer",fontSize:11,lineHeight:1.2}}>✕</button>
                </div>
              )}
            </div>
          ))}
          <button onClick={addDay}
            style={{padding:"7px 12px",borderRadius:10,border:`1.5px dashed ${C.border}`,background:"transparent",color:C.muted,fontSize:12,cursor:"pointer",flexShrink:0,transition:"all .15s",display:"flex",alignItems:"center",gap:4}}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(290px,1fr))",gap:16}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
            {["Todos","Mis ejercicios",...CATEGORIES].map(c=>(
              <button key={c} onClick={()=>setCat(c)} style={{padding:"4px 10px",borderRadius:20,border:`1px solid ${cat===c?C.accent:C.border}`,background:cat===c?"rgba(8,145,178,.12)":"transparent",color:cat===c?C.accent:C.muted,fontSize:11,fontWeight:cat===c?700:400,cursor:"pointer",transition:"all .15s"}}>{c}</button>
            ))}
          </div>

          <div>
            <p style={{fontSize:11,color:C.muted,margin:"0 0 5px"}}>Agregar al bloque · <span style={{color:C.text,fontWeight:600}}>{currentDay.label}</span></p>
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              {BLOCKS.map(b=>{const m=BM[b];return<button key={b} onClick={()=>setAB(b)} style={{padding:"5px 10px",borderRadius:8,border:`1px solid ${activeBlock===b?m.c:C.border}`,background:activeBlock===b?m.bg:"transparent",color:activeBlock===b?m.c:C.muted,fontSize:11,fontWeight:activeBlock===b?700:400,cursor:"pointer",transition:"all .15s"}}>{b}</button>;})}
            </div>
          </div>

          <div style={{overflowY:"auto",maxHeight:460,display:"flex",flexDirection:"column",gap:4}}>
            {filtered.length===0?<p style={{color:C.muted,fontSize:12,textAlign:"center",padding:"16px 0"}}>Sin resultados</p>:filtered.map(ex=>{
              const inBlock=blockOfInDay(ex);
              return(
                <div key={ex.id}
                  style={{display:"flex",alignItems:"center",gap:10,background:inBlock?"rgba(8,145,178,.06)":C.card,border:`1px solid ${inBlock?"rgba(8,145,178,.25)":C.border}`,borderRadius:10,padding:"8px 11px",cursor:"pointer",transition:"all .15s"}}
                  onClick={()=>inBlock?removeEx(ex.id):addEx(ex)}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,marginTop:1}}>
                      ...
                      {ex.video_url&&<a href={ex.video_url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{color:C.accent,fontSize:10,textDecoration:"none"}}>▶ video</a>}
                    </div>
                  </div>
                  {inBlock?(
                    <span style={{fontSize:10,color:BM[inBlock]?.c||C.accent,fontWeight:700,background:(BM[inBlock]?.bg||"rgba(8,145,178,.1)"),border:`1px solid ${BM[inBlock]?.c||C.accent}33`,borderRadius:6,padding:"1px 7px",flexShrink:0}}>✓</span>
                  ):(
                    <span style={{color:C.dim,fontSize:11,flexShrink:0}}>+ agregar</span>
                  )}
                </div>
              );
            })}
        ...
        <div>
          {days.length>1&&(
            <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"10px 12px",marginBottom:12}}>
              <p style={{color:C.muted,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:.7,margin:"0 0 8px"}}>Resumen</p>
              <div style={{display:"flex",flexDirection:"column",gap:3}}>
                {days.map((d,i)=>(
                  <div key={i} onClick={()=>setActiveDay(i)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 8px",borderRadius:7,cursor:"pointer",background:activeDay===i?"rgba(8,145,178,.08)":"transparent",transition:"background .15s"}}>
                    <span style={{fontSize:12,color:activeDay===i?C.accent:C.text,fontWeight:activeDay===i?600:400}}>{d.label}</span>
                    <span style={{fontSize:11,color:d.exercises.length>0?C.success:C.dim}}>{d.exercises.length>0?`${d.exercises.length} ejercicios`:"Vacío"}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{marginBottom:12}}>
            <p style={{fontSize:11,color:C.muted,margin:"0 0 5px"}}>Duración del plan</p>
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              {DURATIONS.map(d=>(
                <button key={d.days} onClick={()=>setDur(d.days)} style={{padding:"4px 10px",borderRadius:20,border:`1px solid ${duration===d.days?C.accent:C.border}`,background:duration===d.days?"rgba(8,145,178,.12)":"transparent",color:duration===d.days?C.accent:C.muted,fontSize:11,fontWeight:duration===d.days?700:400,cursor:"pointer",transition:"all .15s"}}>{d.l}</button>
              ))}
            </div>
            <p style={{color:C.dim,fontSize:11,marginTop:4}}>Vence: <span style={{color:C.accentL}}>{new Date(Date.now()+duration*864e5).toLocaleDateString("es-CO",{day:"numeric",month:"short",year:"numeric"})}</span></p>
          </div>

          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
            <p style={{color:C.text,fontWeight:600,fontSize:13,margin:0}}>{currentDay.label}</p>
            <span style={{color:C.muted,fontSize:12}}>{dayExercises.length} ejercicios</span>
          </div>

          {dayExercises.length===0?(
            <div style={{background:C.surface,border:`1.5px dashed ${C.border}`,borderRadius:12,padding:"24px 16px",textAlign:"center",marginBottom:12}}>
              <p style={{color:C.muted,fontSize:12}}>Selecciona ejercicios de la biblioteca</p>
              <p style={{color:C.dim,fontSize:11,marginTop:4}}>o aplica una plantilla</p>
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:12}}>
              {BLOCKS.map(b=>{
                const exs=dayExercises.filter(e=>(e.block||"Trabajo central")===b);
                  ...
                  <div key={b}>
                    ...
                    {exs.map(ex=>(
                      <div key={ex.id} style={{display:"flex",alignItems:"center",gap:8,background:C.card,border:`1px solid ${C.border}`,borderRadius:9,padding:"7px 10px",marginBottom:5}}>
                        ...
                        <input value={ex.sets} onChange={e=>updateEx(ex.id,"sets",e.target.value)} style={{width:32,background:"transparent",border:`1px solid ${C.border}`,borderRadius:6,padding:"2px 4px",fontSize:12,color:C.accent,fontWeight:700,textAlign:"center",outline:"none"}}/>
                        <span style={{color:C.dim,fontSize:11}}>×</span>
                        <input value={ex.reps} onChange={e=>updateEx(ex.id,"reps",e.target.value)} style={{width:38,background:"transparent",border:`1px solid ${C.border}`,borderRadius:6,padding:"2px 4px",fontSize:12,color:C.accent,fontWeight:700,textAlign:"center",outline:"none"}}/>
                        <button onClick={()=>removeEx(ex.id)} style={{background:"transparent",border:"none",color:C.dim,cursor:"pointer",display:"flex",padding:2,flexShrink:0}}>{I.trash}</button>
                      </div>
                    ))}
                  </div>
                );
          ...
          <button onClick={send} disabled={!totalExercises||loading}
            style={{width:"100%",background:totalExercises?C.accentG:"rgba(255,255,255,.04)",border:"none",borderRadius:10,padding:11,color:totalExercises?"#fff":C.muted,fontWeight:700,fontSize:13,cursor:totalExercises?"pointer":"not-allowed",transition:"all .2s"}}>
            {loading?"Guardando...":`${isEdit?"Actualizar":"Guardar"} plan · ${days.length}d · ${totalExercises} ejercicios`}
          </button>
        </div>
      </div>
    </div>
            ...
            <thead>
              <tr>
                ...
                {days.map((d,i)=>{const isT=localDateStr(d)===localDateStr(new Date());return(
                  <th key={i} style={{padding:"6px 8px",textAlign:"center",color:isT?C.accent:C.muted,fontSize:11,fontWeight:isT?700:500,background:isT?"rgba(8,145,178,.06)":"transparent"}}>
                  ...
                  {days.map((d,j)=>{
                    ...
                    return(
                      <td key={j} onClick={()=>canAdd&&setForm({date:ds,time:hour})}
                        style={{padding:3,verticalAlign:"top",minHeight:36,height:36,borderTop:`1px solid ${C.border}`,cursor:canAdd?"pointer":"default",background:isT?"rgba(8,145,178,.03)":"transparent",transition:"background .15s"}}
                        onMouseEnter={e=>{if(canAdd)e.currentTarget.style.background=isT?"rgba(8,145,178,.08)":"rgba(255,255,255,.02)";}}
                        onMouseLeave={e=>{e.currentTarget.style.background=isT?"rgba(8,145,178,.03)":"transparent";}}>
                        {appts.map(a=>(
...
function MessagesView({user}){
  ...
  const loadThreads=async()=>{
    const{data}=await supabase.from("messages").select("*").eq("therapist_id",user.id).order("created_at",{ascending:false});
          ...
          <div style={{flex:1,overflowY:"auto",padding:"14px 16px",display:"flex",flexDirection:"column",gap:8}}>
            {messages.map(m=>(
              <div key={m.id} style={{display:"flex",justifyContent:m.sender==="therapist"?"flex-end":"flex-start"}}>
                <div style={{maxWidth:"72%",background:m.sender==="therapist"?C.accentG:C.card,border:m.sender==="therapist"?"none":`1px solid ${C.border}`,borderRadius:13,borderBottomRightRadius:m.sender==="therapist"?3:13,borderBottomLeftRadius:m.sender==="therapist"?13:3,padding:"9px 13px",fontSize:13,color:m.sender==="therapist"?"#fff":C.text,lineHeight:1.5}}>
                  {m.content}
                </div>
              </div>
            ))}
...
function AvailabilityView({user}){
  ...
  useEffect(()=>{load();},[]);
  const load=async()=>{const{data}=await supabase.from("availability").select("*").eq("therapist_id",user.id).order("day_of_week").order("start_time");setSlots(data||[]);setLoad(false);};
  const save=async()=>{setSave(true);await supabase.from("availability").insert({...form,therapist_id:user.id,day_of_week:parseInt(form.day_of_week)});setSave(false);load();};
  ...
  return(
    <div style={{maxWidth:700}}>
      <div style={{marginBottom:20}}>
        ...
        <p style={{color:C.muted,fontSize:13,margin:0}}>Define tus horarios de atención.</p>
      </div>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:16,marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10,alignItems:"end"}}>
          <div><label style={{fontSize:11,color:C.muted,display:"block",marginBottom:4}}>Día</label><select value={form.day_of_week} onChange={e=>setF({...form,day_of_week:e.target.value})} style={{...inp}}>{DAYS.map((d,i)=><option key={i} value={i}>{d}</option>)}</select></div>
          <div><label style={{fontSize:11,color:C.muted,display:"block",marginBottom:4}}>Desde</label><input type="time" value={form.start_time} onChange={e=>setF({...form,start_time:e.target.value})} style={{...inp}}/></div>
          <div><label style={{fontSize:11,color:C.muted,display:"block",marginBottom:4}}>Hasta</label><input type="time" value={form.end_time} onChange={e=>setF({...form,end_time:e.target.value})} style={{...inp}}/></div>
      ...
      {loading?<Spinner/>:(
        <div style={{display:"grid",gap:8}}>
          {DAYS.map((day,i)=>{
            const daySlots=byDay[i]||[];if(!daySlots.length)return null;
            return(
              <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
                <div style={{background:C.card,padding:"8px 14px",borderBottom:`1px solid ${C.border}`}}><span style={{color:C.text,fontWeight:600,fontSize:13}}>{day}</span></div>
                {daySlots.map(s=>(
                  <div key={s.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderBottom:`1px solid ${C.border}`,opacity:s.active?1:.5}}>
                    ...
                    <div style={{flex:1}}/>
                    <button onClick={()=>toggle(s.id,s.active)} style={{background:s.active?"rgba(76,175,121,.12)":"rgba(107,115,144,.1)",border:`1px solid ${s.active?"rgba(76,175,121,.3)":C.border}`,borderRadius:8,padding:"4px 10px",color:s.active?C.success:C.muted,fontSize:11,fontWeight:600,cursor:"pointer"}}>{s.active?"Activo":"Inactivo"}</button>
        ...
        <nav style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"8px 0",gap:2}}>
          {NAV.map(item=>{const a=tab===item.id&&!prescribePt&&!profilePt;return(
            <button key={item.id} onClick={()=>{setTab(item.id);goBack();}} title={item.label}
              style={{width:36,height:36,borderRadius:9,border:"none",background:a?"rgba(8,145,178,.15)":"transparent",color:a?C.accent:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .15s"}}>
              {item.icon}
            </button>
          );})}
        </nav>
...
function InviteHandler({token,user}){
  ...
  const register=async()=>{
    ...
    const{data:authData,error:authErr}=await supabase.auth.signUp({email:email.trim(),password:pass,options:{data:{full_name:name.trim()},emailRedirectTo:undefined}});
  ...
  return wrap(
    <>
      ...
      <div style={{display:"grid",gap:9,marginBottom:14}}>
        <div><label style={{fontSize:11,color:C.muted,display:"block",marginBottom:3}}>Nombre completo</label><input value={name} onChange={e=>setName(e.target.value)} placeholder="Tu nombre" style={inp}/></div>
        <div><label style={{fontSize:11,color:C.muted,display:"block",marginBottom:3}}>Correo electrónico</label><input value={email} onChange={e=>setEmail(e.target.value)} placeholder="tu@correo.com" type="email" style={inp}/></div>
        <div><label style={{fontSize:11,color:C.muted,display:"block",marginBottom:3}}>Contraseña</label><input value={pass} onChange={e=>setPass(e.target.value)} placeholder="Mínimo 6 caracteres" type="password" style={inp} onKeyDown={e=>e.key==="Enter"&&register()}/></div>
      </div>
      ...
      <Btn onClick={register} disabled={loading} variant="primary" style={{width:"100%",marginBottom:12}}>{loading?"Creando cuenta...":"Crear cuenta"}</Btn>
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try{
        ...
        if(!existing){await supabase.from("access_requests").insert({user_id:user.id,email:user.email,display_name:meta.full_name||meta.name||""});}
      }catch(e){}
      setRole("pending");
    })();
  },[user]);
